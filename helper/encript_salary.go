...
func getEncryptionKey() ([]byte, error) {
	...
	if len(key) != 32 {
		return nil, errors.New("ENCRYPTION_KEY must be exactly 32 characters for AES-256")
	}
	return []byte(key), nil
}

func EncryptSalary(plainText string) (string, error) {
...
func DecryptSalary(cipherTextB64 string) (string, error) {
	key, err := getEncryptionKey()
	if err != nil {
		return "", err
	}
